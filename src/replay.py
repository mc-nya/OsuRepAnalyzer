...
class Replay(object):
    ...
    def __decode(self, binarystream):
        ...
        while True:
            format_specifier="<b"
            byte = struct.unpack_from(format_specifier, binarystream, self.offset)[0]
    ...
    def parse_string(self, replay_data):
        #print('%X'%replay_data[self.offset])
        format_specifier="<b"
        data = struct.unpack_from(format_specifier, replay_data, self.offset)
        #print data[0]
        if data[0] == 0:
    ...
    def parse_play_data(self, replay_data):
        ...
        if self.game_mode != GameMode.Standard:
            ...
            #print replay_data[self.offset:offset_end]
            #print lzma.decompress_compat(replay_data[self.offset:offset_end])
            datastring = lzma.decompress(replay_data[self.offset:offset_end]).decode('ascii')[:-1]
...
import os
for root,dirs,filenames in os.walk('./replays'):
    for file in filenames:
        print file
        path=os.path.join(root,file)
        replay = parse_replay_file(path)
        dT = [float(event[0]) for event in replay.events]
        X = [float(event[1]) for event in replay.events]
        Y = [float(event[2]) for event in replay.events]
        action = [int(event[3]) for event in replay.events]
        sio.savemat('./replays_mat/'+file+'.mat', {'X': X, 'Y': Y, 'dT': dT, 'act': action})

# replay=parse_replay_file('a.osr')
# dT=[float(event[0]) for event in replay.events]
# X=[float(event[1]) for event in replay.events]
# Y=[float(event[2]) for event in replay.events]
# action=[int(event[3]) for event in replay.events]
# sio.savemat('replay.mat', {'X': X,'Y': Y,'dT': dT,'act': action})
